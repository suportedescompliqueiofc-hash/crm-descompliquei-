...
**Gotcha discovered 2026-08-05 — impersonation never switches the auth session.** `handleImpersonate` (`TabClientesCRM.tsx`) does `UPDATE perfis SET organization_id = targetOrgId WHERE id = user.id` — it mutates the **master admin's own profile row**, `auth.uid()` stays the master's real user id for the rest of the impersonated session. Any table whose RLS/query scopes strictly by `user_id = auth.uid()` (not by `organization_id`) will silently show the master's own (usually empty) rows instead of the impersonated client's — `jornadas` avoids this because its SELECT policy has an explicit `organization_id IN (SELECT organization_id FROM perfis WHERE id = auth.uid())` fallback, and its frontend query (`useJornada.ts`) doesn't filter by `user_id` at all, letting RLS decide. `meus_materiais` didn't have that fallback and got fixed the same way: added policy `"Meus materiais select por organizacao"` (SELECT-only, resolves `user_id IN (SELECT id FROM perfis WHERE organization_id = current profile's organization_id)`) plus dropped the client-side `.eq("user_id", ...)` filter in `useAthosMateriais.ts` (`list` and `useMeusMateriaisAtendimento`). **Not fixed**: `create`/`update`/`delete` on `meus_materiais` still attribute to the master's own `user_id` during impersonation (RLS `with_check`/`qual` still strict `auth.uid() = user_id`) — a material created "as" a client while impersonating is silently owned by the master account, not the client. Any new user-scoped table (`os_conversations`, `os_memories`, etc.) should be checked against this same gotcha before assuming impersonation works for it.
