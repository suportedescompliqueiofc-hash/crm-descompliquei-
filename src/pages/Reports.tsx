      ...
      <Tabs defaultValue="overview" className="space-y-6">
        ...
        <TabsContent value="overview" className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <Card className="border-l-4 border-l-primary"><CardHeader className="pb-3"><CardDescription className="flex items-center gap-2"><Tag className="h-4 w-4" />Novos Leads</CardDescription><CardTitle className="text-3xl font-bold">{reports.kpis.totalNovosLeads + reports.kpis.totalPacientes}</CardTitle></CardHeader></Card>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <Card className="lg:col-span-2"><CardHeader><CardTitle>Captados vs Convertidos</CardTitle><CardDescription>Evolução no período: {periodDisplay}</CardDescription></CardHeader><CardContent><ResponsiveContainer width="100%" height={300}><LineChart data={reports.charts.leadsCapturedData}><CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" /><XAxis dataKey="day" stroke="hsl(var(--muted-foreground))" fontSize={12} /><YAxis stroke="hsl(var(--muted-foreground))" fontSize={12} /><Tooltip contentStyle={chartTooltipStyle} /><Legend /><Line type="monotone" dataKey="captados" stroke="hsl(var(--primary))" strokeWidth={2} name="Captados" /><Line type="monotone" dataKey="convertidos" stroke="hsl(var(--success))" strokeWidth={2} name="Convertidos" /></LineChart></ResponsiveContainer></CardContent>
            <Card><CardHeader><CardTitle>Por Origem</CardTitle><CardDescription>Distribuição de captação</CardDescription></CardHeader><CardContent><ResponsiveContainer width="100%" height={300}><BarChart data={reports.charts.sourceData} layout="vertical"><CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" /><XAxis type="number" stroke="hsl(var(--muted-foreground))" className="text-xs" /><YAxis dataKey="source" type="category" stroke="hsl(var(--muted-foreground))" className="text-xs" width={100} /><Tooltip contentStyle={chartTooltipStyle} /><Bar dataKey="leads" fill="hsl(var(--accent))" name="Leads" /></BarChart></ResponsiveContainer></CardContent>
          </div>
          <Card><CardHeader><CardTitle>Top 10 Criativos por Performance</CardTitle><CardDescription>Campanhas com melhor desempenho</CardDescription></CardHeader><CardContent><Table><TableHeader><TableRow><TableHead>Criativo</TableHead><TableHead>Origem</TableHead><TableHead>Leads Gerados</TableHead><TableHead>Taxa Conversão</TableHead></TableRow></TableHeader><TableBody>{reports.charts.topCreativesData.map((item, i) => <TableRow key={i}><TableCell className="font-medium">{item.name}</TableCell><TableCell><Badge variant="outline">{item.origin}</Badge></TableCell><TableCell>{item.leads}</TableCell><TableCell><Badge className="bg-emerald-100 text-emerald-700 hover:bg-emerald-100">{item.conversion}</Badge></TableCell></TableRow>)}</TableBody></Table></CardContent></Card>
        </TabsContent>
        ...
        <TabsContent value="conversions" className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <Card><CardHeader className="pb-3"><CardDescription className="flex items-center gap-2"><TrendingUp className="h-4 w-4" />Taxa de Conversão</CardDescription><CardTitle className="text-2xl font-bold">{reports.conversions.kpis.conversionRate}%</CardTitle></CardHeader>
            <Card><CardHeader className="pb-3"><CardDescription className="flex items-center gap-2"><DollarSign className="h-4 w-4" />Ticket Médio</CardDescription><CardTitle className="text-2xl font-bold">R$ {reports.conversions.kpis.ticketMedio.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</CardTitle></CardHeader>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
            <Card className="lg:col-span-2"><CardHeader><CardTitle>Conversões por Origem</CardTitle></CardHeader><CardContent><ResponsiveContainer width="100%" height={300}><PieChart><Pie data={reports.conversions.charts.conversoesPorOrigemData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={80} label><Cell fill="hsl(var(--primary))" /><Cell fill="hsl(var(--secondary))" /><Cell fill="hsl(var(--accent))" /><Cell fill="hsl(var(--muted))" /></Pie><Tooltip contentStyle={chartTooltipStyle} /><Legend /></PieChart></ResponsiveContainer></CardContent>
            <Card className="lg:col-span-3"><CardHeader><CardTitle>Valor Convertido</CardTitle><CardDescription>Evolução no período: {periodDisplay}</CardDescription></CardHeader><CardContent><ResponsiveContainer width="100%" height={300}><BarChart data={reports.conversions.charts.valorConvertidoPorDia}><CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" /><XAxis dataKey="day" stroke="hsl(var(--muted-foreground))" className="text-xs" /><YAxis stroke="hsl(var(--muted-foreground))" className="text-xs" /><Tooltip formatter={(value: number) => `R$ ${value.toLocaleString('pt-BR')}`} contentStyle={chartTooltipStyle} /><Bar dataKey="valor" fill="hsl(var(--success))" name="Valor" /></BarChart></ResponsiveContainer></CardContent>
          </div>
        ...
        <TabsContent value="financial" className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <Card><CardHeader><CardTitle>Métodos de Pagamento</CardTitle><CardDescription>Distribuição do faturamento</CardDescription></CardHeader><CardContent><ResponsiveContainer width="100%" height={350}><PieChart><Pie data={reports.financial.metodosPagamentoData} dataKey="value" nameKey="name" cx="50%" cy="50%" innerRadius={60} outerRadius={100} paddingAngle={5}>{reports.financial.metodosPagamentoData.map((entry, index) => <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />)}</Pie><Tooltip formatter={(value: number) => `R$ ${value.toLocaleString('pt-BR')}`} contentStyle={chartTooltipStyle} /><Legend verticalAlign="bottom" height={36}/></PieChart></ResponsiveContainer></CardContent>
          </div>
        </TabsContent>
      </Tabs>
    </div>
